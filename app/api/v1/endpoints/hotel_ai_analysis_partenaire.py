# ══════════════════════════════════════════════════════════════════════════
#  app/api/v1/endpoints/hotel_ai_analysis_partenaire.py
#  Analyse IA des avis clients d'un hôtel via Claude API — ESPACE PARTENAIRE
#
#  POST /hotels/partenaire/{hotel_id}/avis/analyse-ia
#
#  Différence avec hotel_ai_analysis.py (qui est pour l'ADMIN) :
#  - Accessible par PARTENAIRE (ou ADMIN)
...
router = APIRouter(
    prefix="/hotels/partenaire",
    tags=["Partenaire — Analyse IA Avis"],
)
...
@router.post(
    ...
    summary="Générer un rapport IA des avis clients — PARTENAIRE (propriétaire)",
)
async def analyser_avis_ia_partenaire(
    hotel_id: int,
    session:  AsyncSession = Depends(get_db),
    current_user: TokenData = Depends(require_admin_or_partenaire),
):
    """
    Collecte tous les avis d'un hôtel appartenant au partenaire connecté
    et demande à Claude une analyse structurée :
    résumé, points forts/faibles, recommandations, score.

    SÉCURITÉ :
    - Le partenaire ne peut analyser que SES propres hôtels
    - L'admin peut analyser n'importe quel hôtel
    """
    ...
    # ── 2. Vérification de propriété (uniquement pour les partenaires) ──
    if current_user.role == "PARTENAIRE":
        if hotel.id_partenaire != current_user.user_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Cet hôtel ne vous appartient pas"
            )
    ...
    # ── 4. Charger les auteurs en une seule requête ──
    ...
    # ── 6. Prompt optimisé (adapté au partenaire : ton orienté action) ──
    prompt = f"""Tu es un expert en analyse de satisfaction client pour l'hôtellerie.

Tu aides un PARTENAIRE HÔTELIER (propriétaire) à comprendre les retours de ses clients
sur son hôtel "{hotel.nom}" situé à {hotel.ville or ''}, {hotel.pays}.
...
Le rapport doit aider le partenaire à identifier ce qui fonctionne bien, ce qu'il doit
améliorer, et les actions concrètes à entreprendre pour augmenter sa satisfaction client.
...
{{
  ...
  "resume_global": "<paragraphe de 3-4 phrases en français synthétisant la perception générale des clients, ton bienveillant mais honnête>",
...
- Recommandations : 3 à 5 actions concrètes, actionnables et DIRECTEMENT adressées au partenaire (ex: "Améliorer la propreté des salles de bain", "Former le personnel à l'accueil")
...
- Ton : professionnel, bienveillant, constructif (le partenaire doit se sentir accompagné, pas jugé)
"""
    ...
    return AnalyseIAResponse(
        ...
        classification     = ClassificationAvis(**parsed.get("classification", {
            "positif": 0, "neutre": 0, "negatif": 0
        })),
