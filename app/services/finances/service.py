...
RÈGLES MÉTIER :
  1. commission_agence = TAUX % × revenu_hotel
  2. part_partenaire   = revenu_hotel − commission_agence
  3. solde_a_payer     = part calculée − montant_deja_paye
  4. Toutes les agrégations incluent clients ET visiteurs.
...
from sqlalchemy import select, cast, String, func, update, text
...
from app.schemas.finances import (
    ...
    DemandesRetraitResponse, DemandeRetraitItem,
...
# ═══════════════════════════════════════════════════════════
#  HELPER INTERNE — générer numéro de facture + PDF + email
# ═══════════════════════════════════════════════════════════

async def _generer_facture_et_email(
    ...
    """
    Génère le numéro de facture unique (séquence DB),
    crée le PDF et envoie l'email au partenaire.
    Retourne (numero_facture, pdf_bytes).
    """
    from app.models.utilisateur import Partenaire as PartenaireModel
    from app.utils.pdf_generator import generer_facture_paiement_partenaire
    from app.services.email_service import send_paiement_partenaire_email

    # ── 1. Numéro de facture unique ────────────────────────
    seq_result     = await session.execute(
        text("SELECT nextval('voyage_hotel.seq_facture_partenaire')")
    )
    seq_num        = seq_result.scalar_one()
    numero_facture = f"INV-{now.year}-{seq_num:04d}"
    ...
    partenaire_tel    = usr.telephone if usr  else "—"
    nom_entreprise    = part.nom_entreprise if part else "—"

    # ── 3. Générer PDF ─────────────────────────────────────
    pdf_bytes = generer_facture_paiement_partenaire(
        numero_facture    = numero_facture,
        date_paiement     = now,
        partenaire_nom    = partenaire_nom,
        partenaire_prenom = partenaire_prenom,
        partenaire_email  = partenaire_email,
        partenaire_tel    = partenaire_tel or "—",
        nom_entreprise    = nom_entreprise,
        montant           = montant,
        note              = note,
    )

    # ── 4. Envoyer email (non bloquant) ────────────────────
    if partenaire_email:
        _asyncio.create_task(
            send_paiement_partenaire_email(
                to             = partenaire_email,
                prenom         = partenaire_prenom,
                nom            = partenaire_nom,
                nom_entreprise = nom_entreprise,
                numero_facture = numero_facture,
                montant        = montant,
                date_paiement  = now.strftime("%d/%m/%Y"),
                note           = note,
                pdf_bytes      = pdf_bytes,
            )
        )
...
async def get_dashboard(session: AsyncSession) -> FinanceDashboard:
    ...
    total_part = calc_part_partenaire(rev_hotel_global)
...
async def get_revenus(
    ...
    return RevenusResponse(
        ...
        commission_total = calc_commission_agence(total_hotel),
...
def _build_revenu_periode(label: str, rh: float, rv: float, nb: int) -> RevenuPeriode:
    return RevenuPeriode(
        ...
        commission_total = calc_commission_agence(rh),
        nb_reservations  = nb,
    )
...
async def get_reservations_finances_hotel(
    ...
    if statut_commission in ("EN_ATTENTE", "PAYEE"):
        vis_query = sa_text("""
            SELECT rv.id, rv.nom, rv.prenom, rv.email,
            ...
            LEFT JOIN voyage_hotel.commission_visiteur cv
                ON cv.id_reservation_visiteur = rv.id AND cv.id_partenaire = :id_p
    ...
    for v in vis_result.mappings().all():
...
async def payer_partenaire(
    ...
    # ── 2. Taux du partenaire ──────────────────────────────
    part_row = (await session.execute(
        select(PartenaireModel).where(PartenaireModel.id == id_partenaire)
    )).scalar_one_or_none()
    taux = float(part_row.commission) if part_row and part_row.commission else TAUX_COMMISSION_DEFAULT

    # ── 3. Chambres du partenaire ──────────────────────────
    hotel_ids = [r[0] for r in (await session.execute(
    ...
    # ── 7. Marquer commissions clients PAYEE ───────────────
    ...
    montant_final = round(
        solde_reel if solde_reel > 0
        else sum(float(c.montant_partenaire) for c in commissions_clients),
        2
    )

    # ── 9. Générer facture PDF + envoyer email ─────────────
    numero_facture, pdf_bytes = await _generer_facture_et_email(
        session       = session,
        id_partenaire = id_partenaire,
    ...
    # ── 10. Enregistrer le paiement avec la facture ────────
    ...
    return PayerPartenaireResponse(
        success        = True,
        id_partenaire  = id_partenaire,
        message        = f"Paiement de {montant_final:.2f} DT — Facture {numero_facture} générée et envoyée.",
...
#  ✅ Expose id, numero_facture, has_pdf
# ═══════════════════════════════════════════════════════════
...
async def get_demandes_retrait(
    session: AsyncSession,
    statut: Optional[str] = None,
    id_partenaire: Optional[int] = None,
    page: int = 1,
    ...
    from app.models.finances import WithdrawRequest
    from app.models.utilisateur import Partenaire as PartenaireModel

    q = (
        select(WithdrawRequest)
        .join(Utilisateur, Utilisateur.id == WithdrawRequest.id_partenaire)
    )
    if statut:
        q = q.where(cast(WithdrawRequest.statut, String) == statut)
    if id_partenaire:
        q = q.where(WithdrawRequest.id_partenaire == id_partenaire)
    ...
    rows = (await session.execute(
        q.order_by(WithdrawRequest.created_at.desc())
         .offset((page - 1) * per_page)
         .limit(per_page)
    )).scalars().all()
    ...
    for r in rows:
        usr = (await session.execute(
            select(Utilisateur).where(Utilisateur.id == r.id_partenaire)
        )).scalar_one_or_none()

        part = (await session.execute(
            select(PartenaireModel).where(PartenaireModel.id == r.id_partenaire)
        )).scalar_one_or_none()

        items.append(DemandeRetraitItem(
            id                = r.id,
            id_partenaire     = r.id_partenaire,
            partenaire_nom    = usr.nom    if usr else "—",
            partenaire_prenom = usr.prenom if usr else "—",
            partenaire_email  = usr.email  if usr else "—",
            nom_entreprise    = part.nom_entreprise if part else "—",
...
async def valider_demande_retrait(
    ...
    demande = (await session.execute(
        select(WithdrawRequest).where(WithdrawRequest.id == demande_id)
    )).scalar_one_or_none()
    ...
    # ── Générer facture PDF + email ────────────────────────
    numero_facture, pdf_bytes = await _generer_facture_et_email(
        session       = session,
        id_partenaire = demande.id_partenaire,
    ...
    # ── Créer le paiement avec facture ────────────────────
    session.add(PaiementPartenaire(
        id_partenaire  = demande.id_partenaire,
        montant        = demande.montant,
        note           = note_admin or f"Retrait validé — demande #{demande.id}",
        numero_facture = numero_facture,
        pdf_data       = pdf_bytes,
    ))
    await session.commit()
...
async def refuser_demande_retrait(
    ...
    return DemandeActionResponse(
        success = True,
        message = f"Demande #{demande_id} refusée.",
    )